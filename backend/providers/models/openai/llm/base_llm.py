...
class OpenAILikeLargeLanguageModel(OpenAICore, LargeLanguageModel):
    """
    ...
    def _chat_generate(
        ...
        if tools:
            extra_model_kwargs["tools"] = [(PromptMessageFunction(function=tool)).model_dump_json() for tool in tools]
    ...
    def _handle_chat_generate_stream_response(
        ...
        for chunk in response:
            ...
            tool_calls = (
                self._extract_response_tool_calls(assistant_message_tool_calls) if assistant_message_tool_calls else []
            )
            function_call = (
                self._extract_response_function_call(assistant_message_function_call)
            ...
            if len(tool_calls) > 0:
                self._handle_final_tool_calls(final_tool_calls=final_tool_calls, tool_calls=tool_calls)
    ...
    def _handle_final_tool_calls(
        self, final_tool_calls: list[AssistantPromptMessage.ToolCall], tool_calls: list[AssistantPromptMessage.ToolCall]
    ) -> list[AssistantPromptMessage.ToolCall]:
        """
        Handle final tool calls by merging tool_calls into final_tool_calls

        :param final_tool_calls: Accumulated tool calls list
        :param tool_calls: New tool calls to be merged
        :return: Merged tool calls list
        """
        if len(final_tool_calls) == 0:
            return tool_calls

        for tool_call in tool_calls:
            # find matched tool call
            matched_index = -1
            for i, final_call in enumerate(final_tool_calls):
                if hasattr(tool_call, "index") and hasattr(final_call, "index"):
                    if tool_call.index == final_call.index:
                        matched_index = i
                        break
                if tool_call.id == final_call.id:
                    matched_index = i
                    break

            if matched_index >= 0:
                # update final_tool_calls by index
                final_tool_calls[matched_index].function.arguments += tool_call.function.arguments
            else:
    ...
    def _clear_illegal_prompt_messages(self, model: str, prompt_messages: list[PromptMessage]) -> list[PromptMessage]:
        ...
        """
        if model.startswith("o1") or model.startswith("3o"):
            ...
            if message.content is None:
                ...
                for message_content in message.content:
                    if isinstance(message_content, TextPromptMessageContent):
                        ...
                        sub_messages.append(sub_message_dict)
                    elif isinstance(message_content, FilePromptMessageContent):
                        sub_message_dict = {
                            "type": "file",
                            "file": {
                                "data": message_content.data,
    ...
    def _extract_text_from_content_list(self, content_list: list) -> str:
        ...
        return "".join(item["text"] for item in content_list if isinstance(item, dict) and item["type"] == "text")
